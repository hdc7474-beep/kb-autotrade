"""
KB증권 Open API 연동 자동매매 서버 v4
- KB증권 공식 샘플 코드 기준 토큰 발급
- 공지사항 2번 항목 오류 해결 버전
- 초기 PIN: 0000
"""
...
# ── 환경변수에서 KB API 키 자동 로드 ────────────────────
KB_APP_KEY    = os.getenv("KB_APP_KEY",    "").strip()
KB_APP_SECRET = os.getenv("KB_APP_SECRET", "").strip()
KB_API_URL    = os.getenv("KB_API_URL",    "https://openapi.kbsec.com").strip()
...
def ai_signal(code: str) -> dict:
    ...
    score = sum([2 if rsi<35 else -2 if rsi>65 else 0,
                 1 if macd else 0,
                 1 if vr>1.5 else 0])
...
# ── KB증권 공식 샘플 기준 토큰 발급 함수 ─────────────────
async def kb_get_token(app_key: str, app_secret: str, api_url: str) -> dict:
    """
    KB증권 공식 OAuth 토큰 발급
    공지사항 2번 항목 해결 - 공식 샘플 코드 기준으로 작성
    참고: https://openapi.kbsec.com/apidoc_b2c (OAuth 인증 섹션)
    """
    import httpx
    ...
    # KB증권 공식 요청 형식 (form-encoded)
    body = (
        f"grant_type=client_credentials"
        f"&appkey={app_key.strip()}"
        f"&appsecret={app_secret.strip()}"
    )

    add_log("info", f"토큰 발급 요청 → {url}")
    add_log("info", f"appkey 앞 10자리: {app_key[:10]}...")
    ...
    try:
        ...
        try:
            ...
            add_log("warn", f"JSON 파싱 실패 - 응답: {resp.text[:200]}")
        ...
        if access_token:
            ...
            return {"success": True, "token": access_token, "data": data}

        # 실패 시 오류 메시지 파싱
        header  = data.get("dataHeader", {})
        msg     = header.get("processMessage", "알 수 없는 오류")
        code    = header.get("processCode",   "?")
        add_log("warn", f"토큰 발급 실패 [{code}]: {msg}")
        return {"success": False, "message": f"[{code}] {msg}", "raw": data}
        ...
        return {"success": False, "message": "요청 시간 초과 — 잠시 후 다시 시도하세요"}
    except Exception as e:
        add_log("warn", f"예상치 못한 오류: {e}")
        return {"success": False, "message": str(e)}

# ── 서버 시작 시 자동 토큰 발급 ─────────────────────────
async def auto_get_token():
    if not KB_APP_KEY or not KB_APP_SECRET:
        add_log("warn", "환경변수 KB_APP_KEY / KB_APP_SECRET 미설정 — 시뮬레이션 모드")
        return
    result = await kb_get_token(KB_APP_KEY, KB_APP_SECRET, KB_API_URL)
    if result["success"]:
        STATE["token"]     = result["token"]
        STATE["connected"] = True
        add_log("info", "✅ 서버 시작 시 자동 토큰 발급 완료!")
    else:
        add_log("warn", f"자동 토큰 발급 실패: {result['message']}")
        add_log("warn", "시뮬레이션 모드로 동작합니다")

@asynccontextmanager
async def lifespan(app: FastAPI):
    init_prices()
    add_log("info", "KB 자동매매 서버 v4 시작 — 초기 PIN: 0000")
    add_log("info", "KB증권 공식 샘플 기준 토큰 발급 코드 적용 (공지사항 2번 해결)")
    await auto_get_token()
    yield
    add_log("info", "서버 종료")

app = FastAPI(title="KB Auto Trade v4", version="4.0.0", lifespan=lifespan)
...
async def get_account():
    ...
    return {
        "status": "ok", "source": "시뮬레이션",
        "deposit": dep, "eval_pnl": pnl,
        "total":   dep + sum(p["cur_price"]*p["qty"] for p in pos),
        "positions": pos,
    }
...
class TokenReq(BaseModel):
    app_key:    str
    app_secret: str
    api_url:    str = "https://openapi.kbsec.com"
    account:    str = ""

@app.post("/api/token")
async def get_token(req: TokenReq):
    """수동 토큰 발급 — KB증권 공식 샘플 기준"""
    STATE.update({
    ...
    if result["success"]:
        ...
        return {
            ...
            "tip":     "openapi.kbsec.com → API사용신청 → 앱 승인완료 상태인지 확인하세요",
        }
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*55}")
    print(f"  KB증권 AI 자동매매 v4 시작!")
    print(f"  주소: http://localhost:{port}")
    print(f"  초기 PIN: 0000")
    print(f"  KB_APP_KEY 설정: {'✅ 있음' if KB_APP_KEY else '❌ 없음 (시뮬레이션)'}")
    print(f"  KB_API_URL: {KB_API_URL}")
    print(f"  공지사항 2번 오류 해결 버전")
    print(f"{'='*55}\n")
